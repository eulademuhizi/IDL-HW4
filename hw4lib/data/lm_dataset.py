...
class LMDataset(Dataset):
    ...
    def __init__(
        ...
        # Set up data paths
        root_dir = config.get('root_dir', 'hw4_data_subset/hw4p1_data')
        self.text_dir = os.path.join(root_dir, partition)

        # Get all text files in the text directory
        self.text_files = sorted([os.path.join(self.text_dir, f) 
                                for f in os.listdir(self.text_dir) 
                                if f.endswith('.npy')])

        # Take subset if specified in config
        ...
        for file in tqdm(self.text_files):
            ...
            # Create shifted and golden versions
