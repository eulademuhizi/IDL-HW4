...
class ASRTrainer(BaseTrainer):
    ...
    def _train_epoch(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            # Move to device before autocast context
            ...
            with torch.autocast(device_type=self.device):
                ...
                if self.ctc_weight > 0 and not isinstance(ctc_inputs, str):
                    ctc_input, ctc_lengths = ctc_inputs
                    # Add a check for ctc_input type
                    if isinstance(ctc_input, str):
                        ctc_loss = torch.tensor(0.0).to(self.device)
                        loss = ce_loss
                    else:
                        ctc_loss = self.ctc_criterion(ctc_input.log_softmax(-1), targets_golden, ctc_lengths, transcript_lengths)
        ...
        with torch.inference_mode():
            for i, batch in enumerate(dataloader):
                ...
                if recognition_config['beam_width'] > 1:
                    ...
                    seqs, scores = generator.beam_search(prompts, beam_width=recognition_config['beam_width'],temperature=recognition_config['temperature'],
