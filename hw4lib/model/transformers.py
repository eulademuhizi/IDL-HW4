...
class DecoderOnlyTransformer(nn.Module):
    ...
    def _init_(
        ...
        # mapping torken indices to vectors
        ...
        self.dropout = nn.Dropout(dropout)  # Dropout for regula
...
class EncoderDecoderTransformer(nn.Module):
    ...
    def __init__(
        ...
        # encoder layers module list
        self.enc_layers = nn.ModuleList(
            [SelfAttentionEncoderLayer(d_model, num_encoder_heads, d_ff_encoder, dropout) for _ in
             range(num_encoder_layers)])

        # TODO: Create decoder layers
        # Use ModuleList to create a list of decoder layers
        # decode layers module list
        self.dec_layers = nn.ModuleList(
            [CrossAttentionDecoderLayer(d_model, num_decoder_heads, d_ff_decoder, dropout) for _ in
    ...
    def decode(
        ...
        if target_lengths is None:
            target_lengths = torch.full((padded_targets.size(0),), padded_targets.size(1),dtype=torch.long, device=padded_targets.device)
        ...
        for i in range(self.num_decoder_layers):
            if self.training and self.layer_drop_rate > 0 and random.random() < self.layer_drop_rate:
                continue
            # TODO: Pass through decoder layer
            x_dec, self_attn, cross_attn = self.dec_layers[i](x_dec, encoder_output, pad_mask_tgt, pad_mask_src,
                                                              causal_mask)
            # TODO: Save attention weights
