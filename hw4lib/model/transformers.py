...
class EncoderDecoderTransformer(nn.Module):
    ...
    def encode(self, padded_sources: torch.Tensor, source_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, dict]:
        ...
        pad_mask_src = PadMask(x_enc_lengths, max_len=x_enc.size(1)).to(x_enc.device)
        ...
        # Prepare CTC inputs (transposed for CTCLoss)
    ...
    def decode(
        ...
        # Create target padding mask if lengths provided
        pad_mask_tgt = None
        if target_lengths is not None:
            pad_mask_tgt = PadMask(target_lengths, max_len=padded_targets.size(1)).to(padded_targets.device)
    ...
    def score(self, batch_prompts: torch.Tensor, encoder_output: torch.Tensor, pad_mask_src: torch.Tensor) -> torch.Tensor:
        ...
        # Decode with no target lengths (no padding mask for targets)
        seq_out, _ = self.decode(batch_prompts, encoder_output, None, pad_mask_src)
